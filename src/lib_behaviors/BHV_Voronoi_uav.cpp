...
bool BHV_Voronoi::setParam(string param, string param_val) 
{
  ...
  else if(param == "stale_setpt_thresh")
    handled = setNonNegDoubleOnString(m_stale_setpt_thresh, param_val);
...
bool BHV_Voronoi::updateSetPoint()
{
  ...
  if(m_setpt_method == "gridsearch") {
    ...
    auto setpt_staleness = getBufferTimeVal("PROX_SETPT");
    if(setpt_staleness > m_stale_setpt_thresh) {
      postWMessage("Gridsearch setpt info_buffer is stale.");
      return(false);
    }

    std::string gridsearch_str = getBufferStringVal("PROX_SETPT");
    auto pt = string2Point(gridsearch_str);
    if(pt.valid()) {
      gridsearch_setpt = pt;
    }
    else {
