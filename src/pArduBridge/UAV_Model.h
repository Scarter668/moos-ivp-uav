...
public:
  ...
  void registerWarningSystem(std::shared_ptr<WarningSystem> ws) {
  ...
  bool commandGroundSpeed(double speed) {
    return (commandSpeed(speed, SPEED_TYPE::SPEED_TYPE_GROUNDSPEED));
  }
  bool commandAndSetAltitudeAGL(double altitudeAGL_m); // requires guided mode
  bool commandGoToLocationXY(const XYPoint pos,
                             bool holdCurrentAltitudeAGL = false);
  bool commandGoToLocation(const mavsdk::Telemetry::Position &position);

  // void commandGoToLocationXY_async(const XYPoint pos, bool
  // holdCurrentAltitudeAGL = false, std::function<void(bool)> callback =
  // nullptr); void commandGoToLocation_async(const mavsdk::Telemetry::Position
  // &position, std::function<void(bool)> callback = nullptr);

  bool commandReturnToLaunchAsync() const;
  bool commandLoiterAtPos(XYPoint pos, bool holdCurrentAltitude = true);
  bool
  commandAndSetCourse(double heading,
                      bool isAllowed = true); // command COG if in guided mode
  ...
  // Setters
  void setCallbackMOOSTrace(
      const std::function<void(const std::string &)> &callback) {
    callbackMOOSTrace = callback;
  }
  void setCallbackReportEvent(
      const std::function<void(const std::string &)> &callback) {
    callbackReportEvent = callback;
  }
  void setCallbackReportRunW(
      const std::function<void(const std::string &)> &callback) {
    callbackReportRunW = callback;
  }
  void setCallbackRetractRunW(
      const std::function<void(const std::string &)> &callback) {
    callbackRetractRunW = callback;
  }

  void setNextWaypointLatLon(const XYPoint &wp) {
    mts_next_waypoint_coord.set(wp);
  }
  void setLoiterLocationLatLon(const XYPoint &wp) {
    mts_current_loiter_coord.set(wp);
  }
  void setHeadingWyptFromHeading(double heading);
  // void setTargetAltitudeAGL(double altitude) { m_target_altitudeAGL =
  // altitude; }
  ...
  mavsdk::Telemetry::FlightMode getFlightMode() const {
    return (mts_flight_mode.get());
  }
  bool isGuidedMode() const {
    return (mts_flight_mode.get() == mavsdk::Telemetry::FlightMode::Guided);
  }
  bool isHoldCourseGuidedSet() const { return (m_is_hold_course_guided_set); }

  XYPoint getNextWaypointLatLon() const {
    return (mts_next_waypoint_coord.get());
  }
  XYPoint getCourseWaypointLatLon() const {
    return (mts_course_waypoint_coord.get());
  }
  XYPoint getCurrentLoiterLatLon() const {
    return (mts_current_loiter_coord.get());
  }
  ...
  double getMinAirSpeed() const {
    return (mts_polled_params.get().min_airspeed);
  }
  double getMaxAirSpeed() const {
    return (mts_polled_params.get().max_airspeed);
  }
  double getTargetCruiseSpeed() const {
    return (mts_polled_params.get().target_airspeed_cruise);
  }
  double getTargetAirSpeed() const { return (m_target_airspeed); }
  double getAirSpeed() const {
    return (sqrt(mts_velocity_ned.get().north_m_s *
                     mts_velocity_ned.get().north_m_s +
                 mts_velocity_ned.get().east_m_s *
                     mts_velocity_ned.get().east_m_s) +
            mts_velocity_ned.get().down_m_s * mts_velocity_ned.get().down_m_s);
  }
  double getSOG() const { return (m_GPS_SOG_m_s); }
  double getHeading() const {
    return (angle360(mts_attitude_ned.get().yaw_deg));
  }
  double getCOG() const { return (angle360(m_GPS_COG_deg)); }

  double getAltitudeAGL() const {
    return (mts_position.get().relative_altitude_m);
  }
  double getAltitudeMSL() const {
    return (mts_position.get().absolute_altitude_m);
  }
  double getTargetAltitudeAGL() const { return (m_target_altitudeAGL); }
  double getLastSentTargetAltitudeAGL() const {
    return (m_last_sent_altitudeAGL);
  }
...
protected:
  enum class Parameters { AIRSPEED_MIN, AIRSPEED_MAX, AIRSPEED_TARGET_CRUISE };
...
protected:
  ...
  bool commandSpeed(double airspeed_m_s,
                    SPEED_TYPE speed_type = SPEED_TYPE::SPEED_TYPE_GROUNDSPEED);
  ...
  bool commandChangeAltitude_Guided(double altitude_m, bool relativeAlt = false,
                                    double vrate_ms = 0);
  bool commandChangeSpeed_Guided(
      double speed_m_s,
      SPEED_TYPE speed_type = SPEED_TYPE::SPEED_TYPE_AIRSPEED);
  bool commandChangeHeading_Guided(
      double hdg_deg, HEADING_TYPE hdg_type = HEADING_TYPE_COURSE_OVER_GROUND);
...
protected:
  ...
  // If hold heading is set, it will ignore further repositions command, and
  // Guided mode has to be exited
...
make_mission_item_wp(float latitude_deg1e7, float longitude_deg1e7,
                     ...
                     MAV_CMD command, float p2 = 0, float p3 = 0);

bool create_missionPlan(
    std::vector<mavsdk::MissionRaw::MissionItem> &mission_plan,
    double lat_deg_home = -35.359833, double lon_deg_home = 149.164703);
